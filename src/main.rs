...
fn main() {
    let (block, plot_points, halvings) = simulate();

    print_halving(halvings);

    let years = block as f64 / BLOCKS_PER_YEAR as f64;

    generate_plots(plot_points, years);
}

fn simulate() -> (u32, Vec<(f64, f64)>, Vec<i64>) {
    ...
    while available_subsidies > 0 {
        ...
        println!(
            "Block {} (~{:.2} years): Block subsidy: {} (~{} ZEC), ZSF balance: {} (~{} ZEC)",
            block,                                  // current block
